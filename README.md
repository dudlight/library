...
Sub Refresh_Active_Sheet_PivotTables()
...
'Options:
'choose multiple tabs to print
'Set file names as string, cell reference, or combination of both
'Save PDF in subfolder.
...
'INSTRUCTION: By default, the macro prints the active sheet. If printing other sheets, use these lines of code:
'Sheets(Array("WorksheetName1", "WorksheetName2", "WorksheetName3", "WorksheetName4")).Select


'INSTRUCTION: Use one of the following lines to set the filename. The default uses the Excel file's name
fileName = Left(ThisWorkbook.Name, Len(ThisWorkbook.Name) - 5)
'fileName = "Enter File Name Here"
'fileName = ActiveSheet.Range("A1")
'fileName = Worksheets("SheetName").Range("A1")&" Enter additional text here"

'this line removed forbidden characters from the filename and replaces with underscore
For Each v In Array("/", "\", "|", ":", "*", "?", "<", ">", """")
            fileName = Replace(fileName, v, "_")
            Next

'INSTRUCTION: If you want to save in a subfolder enter the path here
'subFolderPath = "\subfolder1\subfolder2\subfolder3\"

saveString = ThisWorkbook.Path & subFolderPath & fileName

'These two lines for debugging only.
'Range("A1") = fileName
'Range("A21") = subFolderPath
'Range("a3") = saveString
...
Sub SingleLadies() 'Macro for Projects that only have one charge line

If Sheets("Setup").Range("C50").Value = "Phase Name" & Sheets("Setup").Range("C50").Value <> "Phase Name" Then 'Looks to see if this is a details by labor category
GoTo Line1
Else
End If
If Sheets("Setup").Range("D50").Value = "Phase Name" & Sheets("Setup").Range("D50").Value <> "Phase Name" Then 'Looks to see if this is a details by employee name
GoTo Line2
Else
Exit Sub
End If
...
If Sheets("Setup").Range("C50").Value = "Phase Name" Then
    Worksheets("Details by Labor Category").Range("M3").Value = Worksheets("Setup").Range("C49").Value
'Worksheets("Setup").Range("C49").Copy
'ActiveSheet.Paste Destination:=Worksheets("Details by Labor Category").Range("M3")
    Sheets("Details by Labor Category").Rows(5 & ":" & 54).Hidden = False
...
If Sheets("Setup").Range("D50").Value = "Phase Name" Then
    Worksheets("Details by Labor Category").Range("M3").Value = Worksheets("Setup").Range("D49").Value
    Sheets("Details by Labor Category").Rows(5 & ":" & 54).Hidden = False
...
If Sheets("Setup").Range("C50").Value = "Phase Name" & Sheets("Setup").Range("C50").Value <> "Phase Name" Then
Sheets("Details by Labor Category").Rows(5 & ":" & 54).Hidden = False
        Else
If Sheets("Setup").Range("D50").Value = "Phase Name" & Sheets("Setup").Range("D50").Value <> "Phase Name" Then
Sheets("Details by Labor Category").Rows(5 & ":" & 54).Hidden = False
        Else
        Exit Sub
End If
